...
theme.Cart = (function() {
  var selectors = {
    ...
    mulberryWarrantyDetails: '[mulberry-inline-data]',
  ...
  var attributes = {
    ...
    mulberryId: 'data-mulberry-variant-id'
  };
  ...
  function Cart(container) {
    ...
    if (this.ajaxEnabled) {
      ...
      this._createMulberryCartListener();
    }
  }
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _updateItemQuantity: function(
      ...
      var id = $itemElement.attr(attributes.cartItemId);
      var params = {
        url: '/cart/change.js',
          ...
          function(state) {
            if (state.item_count === 0) {
              ...
              if (value === 0) {
                ...
                if (window.mulberryShop && window.mulberryShop.cart) {
                  mulberryShop.removeDetachedWarranty(state)
                    .then(newCart => {
                      mulberryShop.cart.syncQuantities(newCart)
                      .then(content => {
                        var $lineItem = $('[data-cart-item-key="' + key + '"]');
                        var item = this.getItem(key, content);
    ...
    _createCart: function(state) {
      ...
      if (window && window.mulberryShop) {
		mulberryShop.cart.render({refresh: true})      
      }
    },
    
    _removeAndSyncQuantities: function() {
      let take = this
  		 fetch('/cart.js', {
            ...
            cart.json().then(function (firstCart) {
              mulberryShop.removeDetachedWarranty(firstCart)
              .then(newCart => {
                mulberryShop.cart.syncQuantities(newCart)
                .then(content => {
                  if (firstCart.item_count != content.item_count || firstCart.total_price != content.total_price ) {
        			take._createCart(content);
                  }
                })
              })
            });
    ...
    _createMulberryCartListener: function() {
        let temp = this
       document.addEventListener('mulberry-shopify:loaded', () => {
        if (window.location.href.indexOf('/cart') != -1) {
        // sync quantities on first load of cart page
          temp._removeAndSyncQuantities()
        }
      })
    ...
    _createLineItemList: function(state) {
      return $.map(
        ...
        function(item, index) {
          ...
          if (window.mulberryShop && window.mulberryShop.cart) {
            this._setMulberryId($item, item.id)          
          }
    ...
    _setLineItemAttributes: function($item, item, index) {
      $item
        ...
        .attr(attributes.cartItemId, item.id)
        .attr(attributes.mulberryId, item.id);
      if (window.mulberryShop && window.mulberryShop.cart && item.vendor == 'Mulberry') {
            $item
    ...
    _setMulberryId: function($item, id) {
      $(selectors.mulberryWarrantyDetails, $item).html(`<warranty-details data-variant-id="${id}"></warranty-details>`);
    },
    ...
    _onRemoveItem: function(evt) {
      ...
      var id = $lineItem.attr(attributes.cartItemId);
      this._hideCartError();
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _completeMulberryModal: function() {
      window.skip = true
      $(this.selectors.productForm, this.$container).submit();
    },

    _initMulberry: function(take) {
      (async function () {
              $('body').on('mulberryTriggerModalComplete', () => {
                take._completeMulberryModal()           
              })
        ...
        const offer = await mulberry.core.getWarrantyOffer({
          id: product.id,
          title: product.title,
          price: product.variants[0].price, // Only 1 variant
          detail: product
        })
        
        if (offer.length == 0) {
          window.pdpModal = null
        	return
        }
        
        let pdpInline = await mulberry.inline.init({
          offers: offer, 
          settings: mulberry.core.settings,
          selector: '#mulberry-inline-target',
          onWarrantyToggle: async (warranty) => {
            if (warranty.isSelected) {
              window.selectedWarranty = warranty.offer
            } else {
              window.selectedWarranty = null                                                                                
            }
          },
        })
        
        let pdpModal = await mulberry.modal.init({
          offers: offer, 
          settings: mulberry.core.settings,
          onWarrantySelect: async (warranty) => {
            const result = await mulberryShop.getWarrantyVariant(warranty);
            await mulberryShop.addWarrantyToCart(result.shopify_variant_id, {title: product.title, variantId: product.variants[0].id}, warranty);
            window.selectedWarranty = null
            $( "body").trigger( "mulberryTriggerModalComplete", [ "Custom", "Event" ] );
    ...
    _addWarrantyFirst: function() {
      return new Promise(async (resolve, reject) => {
          let response = await mulberryShop.getProductData(mulberryShop.getProductUrl())
          let product = response.product;
      
          const result = await mulberryShop.getWarrantyVariant(window.selectedWarranty);
          await mulberryShop.addWarrantyToCart(result.shopify_variant_id, {title: product.title, variantId: product.variants[0].id}, window.selectedWarranty);
          resolve(result);
      });
    },
    ...
    _initAddToCart: function() {
      $(this.selectors.productForm, this.$container).on(
        'submit',
        function(evt) {
          if (window.pdpModal && window.selectedWarranty) {
            evt.preventDefault();
            this._addWarrantyFirst()
              .then(res => {
                window.selectedWarranty = null
                window.skip = true
          ...
          } else if (window.pdpModal && !window.skip) {
            evt.preventDefault();
